...
import { Divider, Modal, Typography, TextField, Checkbox, Button, Tab, Box, Tabs } from "@mui/material";
...
function UpdatePoolModal() {
    ...
    useEffect(() => {
        ...
        setSelectedTiles(editingPool.tiles?.map(s => s.id) ?? []);
    ...
    const handleDelete = async () => {
        ...
        if (!confirm(`Вы уверены, что хотите удалить бассейн "${editingPool.name}"?`)) return;
        ...
        <Modal open={updatePoolModal} onClose={() => setUpdatePoolModal(false)}>
            <ModalBox className="relative w-5xl! overflow-hidden">
                ...
                <div className="flex justify-end gap-2">
                    <Button variant="outlined" color="error" className="mr-auto!"
                        onClick={handleDelete}
                    >
                        Удалить
                    </Button>
                    <Button variant="outlined"
                        onClick={() => setUpdatePoolModal(false)}
                    >
                        Отмена
                    </Button>
                    <Button
